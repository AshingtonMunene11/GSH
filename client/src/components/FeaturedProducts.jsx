...
export default function FeaturedProducts({ count = 4 }) {
  ...
  const shuffled = [...featured].sort(() => 0.5 - Math.random()).slice(0, count);
        ...
        {shuffled.map((p) => (
          ...
          >
            ...
            <div className="flex flex-col flex-grow p-4">
              <h3 className="text-lg font-bold mb-2 text-gray-700 line-clamp-1">{p.name}</h3>
              <p className="text-sm text-gray-600 mb-2 line-clamp-2">{p.short_description}</p>
              <p className="text-base font-semibold text-gray-900 mb-3">${p.price}</p>
